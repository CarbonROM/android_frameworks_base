...
public class ThemeOverlayController extends SystemUI implements Dumpable {
    ...
    protected @Nullable FabricatedOverlay getOverlay(int color, int type) {
        ...
        List<Integer> allNShades = new ArrayList<>();
        List<Integer> allAShades = new ArrayList<>();
        allAShades.addAll(a1Shades);
        allAShades.addAll(a2Shades);
        allAShades.addAll(a3Shades);
        allNShades.addAll(n1Shades);
        allNShades.addAll(n2Shades);

        int paletteSize = a1Shades.size();

        List<Integer> allShades = type == ACCENT ? allAShades : allNShades;
