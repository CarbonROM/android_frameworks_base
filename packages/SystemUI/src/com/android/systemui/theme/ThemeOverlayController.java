...
public class ThemeOverlayController extends SystemUI {
    ...
    static final String OVERLAY_CARBON_BLACK_THEME =
            "org.carbonrom.overlay.android.blacktheme";
    static final String OVERLAY_CARBON_BLACK_THEME_SYSUI =
    ...
    private void applyBlackTheme(boolean state) {
        ...
        try {
            mOverlayManager.setEnabled(OVERLAY_CARBON_BLACK_THEME, state, userId);
            mOverlayManager.setEnabled(OVERLAY_CARBON_BLACK_THEME_SYSUI, state, userId);
            if (DEBUG) {
                Log.d(TAG, "applyBlackTheme: overlayPackage="
                        + OVERLAY_CARBON_BLACK_THEME + " userId=" + userId);
                Log.d(TAG, "applyBlackTheme: overlayPackage="
                        + OVERLAY_CARBON_BLACK_THEME_SYSUI + " userId=" + userId);
            }
        } catch (Exception e) {
            Log.e(TAG, "Failed to " + (state ? "enable" : "disable")
                    + " overlay " + OVERLAY_CARBON_BLACK_THEME + " for user " + userId);
            Log.e(TAG, "Failed to " + (state ? "enable" : "disable")
                    + " overlay " + OVERLAY_CARBON_BLACK_THEME_SYSUI + " for user " + userId);
        }
    }
