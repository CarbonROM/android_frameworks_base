...
public class DataSwitchTile extends QSTileImpl<BooleanState> {
        ...
        if (!mCanSwitch) {
            ...
            AsyncTask.execute(new Runnable() {
                public final void run() {
                    setMobileDataEnabled(1 - mSubscriptionManager.getDefaultDataPhoneId());
                    refreshState();
                }
            });
        }
     ...
     * subscription
     */
    private void setMobileDataEnabled(int subId) {
        final TelephonyManager telephonyManager =
                TelephonyManager.from(host.getContext()).createForSubscriptionId(subId);
        ...
        if (subInfoList != null) {
            for (SubscriptionInfo subInfo : subInfoList) {
                ...
                if (subInfo.getSubscriptionId() != subId && !subInfo.isOpportunistic()) {
                    TelephonyManager.from(host.getContext()).createForSubscriptionId(
