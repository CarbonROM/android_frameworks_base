...
public class SmartPixelsTile extends QSTileImpl<BooleanState> {
    ...
    public void handleClick() {
        ...
        mLowPowerMode = Settings.Global.getInt(
                mContext.getContentResolver(), Settings.Global.LOW_POWER_MODE, 0);
        if (mSmartPixelsOnPowerSave == 0) {
            if (mSmartPixelsEnable == 0) {
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        if ((mSmartPixelsEnable == 0) && (mLowPowerMode == 0)) {
            state.label = mContext.getString(R.string.quick_settings_smart_pixels);
            state.value = false;
        } else {
            state.value = true;
            if ((mLowPowerMode != 0) && (mSmartPixelsOnPowerSave != 0)) {
