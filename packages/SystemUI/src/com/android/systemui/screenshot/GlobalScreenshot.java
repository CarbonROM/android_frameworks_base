...
class GlobalScreenshot {
    ...
    void takeScreenshotExtended(final Runnable finisher, final boolean statusBarVisible,
        ...
        while(images.size() < MAXSHOTS && !isDuplicate) {
            ...
            //current = current.copy(Bitmap.Config.ARGB_8888, true);
            if (current == null) {
                notifyScreenshotError(mContext, mNotificationManager,
                        R.string.screenshot_failed_to_capture_text);
                finisher.run();
    ...
    Bitmap mergeImages(ArrayList<Bitmap> images, int width, int height) {
        Log.w(TAG, "Merging images!");
        Bitmap mergedImage = Bitmap.createBitmap(width, height, 
Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(mergedImage);
        canvas.setHwBitmapsInSwModeEnabled(true);
        Paint paint = new Paint();
        for(int i = 0; i < images.size(); i++) {
            canvas.drawBitmap(images.get(i), images.get(i).getWidth(), images.get(i).getHeight(), paint);
