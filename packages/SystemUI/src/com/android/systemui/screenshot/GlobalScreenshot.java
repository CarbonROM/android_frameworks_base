...
class GlobalScreenshot {
    ...
    void takeScreenshotExtended(final Runnable finisher, final boolean statusBarVisible,
        ...
        int masterHeight = 0;
        int maxPosition = displayHeight * 5;  //5 * 2340 = 11700
        ...
        while(images.size() < MAXSHOTS && !isDuplicate) {
            ...
            if(masterHeight + displayHeight - statHeight >= maxPosition) {
                ...
                current = SurfaceControl.screenshot(bottomRect, displayWidth, displayHeight, rot);
            } else if(images.size() == 0) {   //first screenshot, ignore nav bar
                Log.w(TAG, "first shot");
                current = SurfaceControl.screenshot(topRect, displayWidth, displayHeight, rot);
            } else {    //body screenshot
                Log.w(TAG, "body shot");
                current = SurfaceControl.screenshot(regularRect, displayWidth, displayHeight, rot);
            }
