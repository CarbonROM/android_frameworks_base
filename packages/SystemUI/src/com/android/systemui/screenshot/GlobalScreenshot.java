...
class GlobalScreenshot {
        ...
        for (int i = 0; ; i++) {
            if (expanded) {
                try {   //delay screenshots
                    if(i == 0)  //slow to get to main screen
                        Thread.sleep(3000);
                    else
                        Thread.sleep(3300); //speed this up on more screensshots
    ...
    Bitmap mergeBitmapVertical(ArrayList<Bitmap> imagesToStitch){
        ...
        if(imagesToStitch.size() == 1)
            ...
            for (int i = 0; i < imagesToStitch.size() - 1; i++) {
    ...
    Bitmap stripNavStatBar(Bitmap bm, boolean navBar, boolean statBar,
        ...
        if(navBar && statBar) {
            tempStitch = Bitmap
                    .createBitmap(bm, 0,
                            statHeight * 2, mDisplayMetrics.widthPixels,
                            mDisplayMetrics.heightPixels - (navHeight * 2));
            bm.recycle();
            ...
            tempStitch = Bitmap
                    .createBitmap(bm, 0, statHeight * 2, mDisplayMetrics.widthPixels,
