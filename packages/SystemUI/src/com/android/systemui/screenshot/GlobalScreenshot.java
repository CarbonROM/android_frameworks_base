...
class GlobalScreenshot {
    ...
    void takeScreenshotExtended(final Runnable finisher, final boolean statusBarVisible,
        ...
        Rect bottomRect = new Rect(0, statHeight, displayWidth, displayHeight);
        ...
        if(!viewSupported()) {    //checks if view is supported
            notifyScreenshotError(mContext, mNotificationManager,
                    R.string.screenshot_extended_not_supported);
            finisher.run();
        }

        while(images.size() < MAXSHOTS && !isDuplicate) {
            ...
            try {
                //moveScreen(displayWidth, displayHeight);
                Thread.sleep(2100);
