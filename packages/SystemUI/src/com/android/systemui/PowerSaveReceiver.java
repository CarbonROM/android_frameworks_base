...
public class PowerSaveReceiver extends BroadcastReceiver {
    ...
    private boolean mEcoDisplayRunning;
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        public void update() {
            ...
            Intent ecoDisplay = new Intent(mContext, com.android.systemui.ecodisplay.EcoDisplayService.class);
            if (Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.ECODISPLAY_ON_POWER_SAVE,
               0, mCurrentUserId) != 0) {
                if (pm.isPowerSaveMode() && (Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.ECODISPLAY_ENABLE, 0,
                  mCurrentUserId) == 0) && !mEcoDisplayRunning) {
                    mContext.startService(ecoDisplay);
                    mEcoDisplayRunning = true;
                    Log.d(TAG, "Started EcoDisplay");
                } else if (!pm.isPowerSaveMode() && (Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.ECODISPLAY_ENABLE, 0,
                  mCurrentUserId) == 0) && mEcoDisplayRunning){
                    mContext.stopService(ecoDisplay);
                    mEcoDisplayRunning = false;
                    Log.d(TAG, "Stopped EcoDisplay");
                }
            } else if ((Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.ECODISPLAY_ON_POWER_SAVE, 0,
              mCurrentUserId) == 0) && mEcoDisplayRunning){
                mContext.stopService(ecoDisplay);
                mEcoDisplayRunning = false;
                Log.d(TAG, "Stopped EcoDisplay");
