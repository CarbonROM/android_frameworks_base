...
implements DozeReceiver {
    ...
    private void updateBottomPadding() {
        ...
        if (this.mAmbientIndication.getVisibility() == 0) {
            n = this.mStatusBar.getNotificationScrollLayout().getBottom() - this.getTop();
        }
        notificationPanelView.setAmbientIndicationBottomPadding(n);
    }
    ...
    public void setIndication(CharSequence charSequence, PendingIntent pendingIntent) {
        ...
        charSequence = this.mAmbientIndication;
        if (!(bl ^ true)) {
            n = 4;
        }
        charSequence.setVisibility(n);
        this.updateBottomPadding();
    }
