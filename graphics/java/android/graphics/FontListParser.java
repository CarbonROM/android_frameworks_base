...
public class FontListParser {
    ...
    public static FontConfig parse(File configFilename, String fontDir) throws XmlPullParserException, IOException {
        try (InputStream is = new FileInputStream(configFilename)) {
            ...
            parser.nextTag();
            if (DEBUG) Slog.d(TAG, "[FontConfig parse] fontDir=" + fontDir);
    ...
    private static @Nullable FontConfig.Font readFont(
        ...
        if (DEBUG) Slog.d(TAG, "[readFont] filePath= " + filePath + " originalPath= " + originalPath);
    ...
    private static String findUpdatedFontFile(String psName,
        ...
        if (updatableFontMap != null) {
            if (DEBUG) Slog.d(TAG, "[findUpdatedFontFile] updatableFontMap is not null");
            File updatedFile = updatableFontMap.get(psName);
            if (DEBUG) Slog.d(TAG, "[findUpdatedFontFile] updatedFile= " + updatedFile);
            if (updatedFile != null) {
