...
public class CarbonGesturesListener implements PointerEventListener {
    ...
    public CarbonGesturesListener(Context paramContext, Callbacks callbacks) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mSwipeMask == GESTURE_TWO_SWIPE_MASK) {
                    mSwipeMask = 1;
                    double sizeInX = mDownX[0] - event.getX(0);
                    double sizeInY = mDownY[0] - event.getY(0);
                    double abssizeInX = Math.abs(sizeInX);
                    double abssizeInY = Math.abs(sizeInY);
                    if (abssizeInY > abssizeInX) {
                        if (sizeInY > 0) {
                            mCallbacks.onSwipeUpTwoFinger();
                            // we only want upwards swipes to trigger things
                            if (DEBUG) Slog.d(TAG, "Swipe Up Detected");
                        }
                    } else {
                        if (sizeInX < 0) { // swipe right
                            mCallbacks.onSwipeRightTwoFinger();
                            if (DEBUG) Slog.d(TAG, "Swipe Right Detected");
                        } else { // swipe left
                            mCallbacks.onSwipeLeftTwoFinger();
                            if (DEBUG) Slog.d(TAG, "Swipe Left Detected");
                        }
                    }
                }
                break;
    ...
    private void detectSwipe(int i, long time, float x, float y) {
        ...
        if (mSwipeMask < GESTURE_TWO_SWIPE_MASK
                && (y > fromY + SWIPE_DISTANCE || x > fromX + SWIPE_DISTANCE)
    ...
    interface Callbacks {
        void onSwipeRightTwoFinger();
        void onSwipeLeftTwoFinger();
