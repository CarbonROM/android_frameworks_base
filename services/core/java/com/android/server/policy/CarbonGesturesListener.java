...
public class CarbonGesturesListener implements PointerEventListener {
    ...
    private void detectSwipe(MotionEvent move) {
        ...
        for (int p = 0; p < pointerCount; p++) {
            ...
            final int i = findIndex(pointerId);
            if (i != UNTRACKED_POINTER 
                    && validSwipeDirection(i, move.getX(p), move.getY(p))) {
                detectSwipe(i, move.getEventTime(), move.getX(p), move.getY(p));
            }
        }
    }

    private boolean validSwipeDirection(int i, float x, float y) {
        boolean valid = true;
        float sizeInX = mDownX[i] - x;
        float sizeInY = mDownY[i] - y;
        float abssizeInX = Math.abs(sizeInX);
        float abssizeInY = Math.abs(sizeInY);
        switch (N_SWIPE_DIRECTION) {
            case UP:
            if (!(abssizeInY > abssizeInX && sizeInY > 0)) {
                if (DEBUG) Slog.d(TAG, "invalid UP Gesture on Pointer: "+ i);
                valid = false;
            }
            break;
            case DOWN:
            if (!(abssizeInY > abssizeInX && sizeInY <= 0)) {
                if (DEBUG) Slog.d(TAG, "invalid DOWN Gesture on Pointer: "+ i);
                valid = false;
            }
            break;
            case LEFT:
            if (!(abssizeInY <= abssizeInX && sizeInX > 0)) {
                if (DEBUG) Slog.d(TAG, "invalid LEFT Gesture on Pointer: "+ i);
                valid = false;
            }
            break;
            case RIGHT:
            if (!(abssizeInY <= abssizeInX && sizeInX <= 0)) {
                if (DEBUG) Slog.d(TAG, "invalid RIGHT Gesture on Pointer: "+ i);
                valid = false;
            }
            break;
            default:
